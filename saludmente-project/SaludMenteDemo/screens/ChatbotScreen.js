...
export default function ChatbotScreen() {
  ...
  const getResponse = (userMessage) => {
    ...
    if (
      message.includes("ansi") ||
      message.includes("nervios") ||
      message.includes("miedo") ||
      message.includes("preocup")
    ) {
    ...
    if (
      message.includes("respir") ||
      message.includes("calmar") ||
      message.includes("relaj")
    ) {
    ...
    if (
      message.includes("bonita") ||
      message.includes("psicologa") ||
      message.includes("quien")
    ) {
      return predefinedResponses.naty[
        Math.floor(Math.random() * predefinedResponses.naty.length)
      ];
    }

    if (
      message.includes("trist") ||
      message.includes("deprim") ||
      message.includes("llor")
    ) {
    ...
    if (
      message.includes("estr") ||
      message.includes("agobiad") ||
      message.includes("abrumad") ||
      message.includes("presión")
    ) {
    ...
    if (
      ...
      message.includes("apoyo") ||
      message.includes("acompañ")
    ) {
        ...
        {isTyping && (
          <View style={[styles.messageWrapper, styles.botMessageWrapper]}>
            <Avatar.Icon
              size={32}
              icon="robot"
              style={[styles.botAvatar, { backgroundColor: colors.education }]}
            />
            <Card style={[styles.messageCard, styles.botMessage]}>
              <Card.Content style={styles.messageContent}>
                <Text
                  style={[styles.typingText, { color: colors.text.secondary }]}
                >
                  Escribiendo...
                </Text>
              </Card.Content>
